...
bool NavigationGrid::FindPath(const Vector3& from, const Vector3& to, NavigationPath& outPath) {
	//work out which node 'from' sits in, end 'to' sits in
	...
	if (toX<0 || toX>gridWidth - 1 || toZ<0 || toZ>gridHeight - 1) {
		return false; //out of map region
	}
	...
	while (!openList.empty()) {
		...
		if (currentBestNode == endNode) { //path found!
			...
			for(int i = 0; i <4; ++i){
				...
				//might be better route to this node
	...
	return false; // openlist emptied out with no path!
}
