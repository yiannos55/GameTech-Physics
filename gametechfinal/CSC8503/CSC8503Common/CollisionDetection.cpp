...
bool CollisionDetection::RayBoxIntersection(const Ray&r, const Vector3& boxPos, const Vector3& boxSize, RayCollision& collision) {
	...
	const float epsilon = 0.0001f; //ammount of leeway in our calcs
	for (int i = 0; i < 3; ++i) {
...
bool CollisionDetection::RaySphereIntersection(const Ray&r, const Transform& worldTransform, const SphereVolume& volume, RayCollision& collision) {
	...
	float sphereRadious = volume.GetRadius();

	if (r.GetPosition() == spherePos)
	{
		return false;
	}

	//get the direction between ray origin and sphere origin!
	Vector3 dir = (spherePos - r.GetPosition());
	
	//project the sphere's origin onto the ray direction vector
	...
	if (sphereDist > sphereRadious) {
		return false;
	}

	float offset = sqrt((sphereRadious * sphereRadious) - (sphereDist * sphereDist));
